...
export function PokemonCard({ pokemon }: { pokemon: PokemonResponse }) {
  ...
  return (
    ...
    >
      ...
      <Link href={`/pokemon/${pokemon.id}`} className="block relative z-0 group-hover:scale-105 transition-transform duration-300">
      ...
      <div className="flex items-center justify-between border-t border-border pt-4 px-2">
        ...
        >
          <ThumbsUp className={cn("w-5 h-5", pokemon.userVote === 1 && "fill-emerald-600")} /> 
        ...
        >
          <ThumbsDown className={cn("w-5 h-5", pokemon.userVote === -1 && "fill-rose-600")} /> 
